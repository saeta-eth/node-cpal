...
const PACKAGE_TARGETS = Object.freeze({
  ...
  '@node-cpal/backend-jack': new Set([
    'darwin-x64',
    'darwin-arm64',
    'linux-x64',
    'linux-arm64',
  ...
  '@node-cpal/backend-pipewire': new Set(['linux-x64', 'linux-arm64']),
  '@node-cpal/backend-pulseaudio': new Set(['linux-x64', 'linux-arm64']),
  '@node-cpal/backend-asio': new Set(['win32-x64']),
});
...
function loadBinding() {
  ...
  const supportedTargets = PACKAGE_TARGETS[PACKAGE_NAME] || PACKAGE_TARGETS['node-cpal'];
  if (!supportedTargets.has(target)) {
    throw new CpalError(
      'UNSUPPORTED_OPERATION',
      `${PACKAGE_NAME} does not publish a native binary for ${target}`,
      { operation: 'loadBinding' }
    );
  }
  ...
  try {
    try {
      return require(path.join(__dirname, 'bin', target, 'index.node'));
    ...
    throw new CpalError(
      'BINDING_LOAD_FAILED',
      `Failed to load ${PACKAGE_NAME} binary for ${target}: ${error.message}`,
...
if (ALL_HOSTS.some((id) => id.toString() === 'jack')) {
  module.exports.JackHost = JackHost;
}
if (ALL_HOSTS.some((id) => id.toString() === 'pipewire')) {
  module.exports.PipeWireHost = PipeWireHost;
}
if (ALL_HOSTS.some((id) => id.toString() === 'custom')) {
  module.exports.CustomHost = CustomHost;
  module.exports.CustomDevice = CustomDevice;
  module.exports.CustomStream = CustomStream;
}
